...
const express = require('express');
const server = express();
const brainWaveDataFile = path.join(__dirname, 'brainWaveData.json');

let brainWaveData = [];

// Load existing data from file on server start
try {
    const fileData = fs.readFileSync(brainWaveDataFile, 'utf8');
    brainWaveData = JSON.parse(fileData);
} catch (err) {
    console.error('Error reading brainWaveData file:', err);
}
...
server.post('/api/postbrain', (req, res) => {
    ...
    fs.writeFileSync(brainWaveDataFile, JSON.stringify(brainWaveData), 'utf8');
